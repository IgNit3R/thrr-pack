...
bool PutFile(std::shared_ptr<const LIST> file_data, const std::vector<unsigned char> &data, const std::vector<std::shared_ptr<LIST> > &, std::shared_ptr<FILE>) {
	...
	if(strcmp(file_ext, ".anm") == 0) {
		...
		if(strcmp(filename, "mission.msg") == 0) {
			return th125mission_convert(file_data, data);
		} else if(filename[0] == 'e' || ::strcmp(filename, "staff.msg") == 0) {
			return Touhou::Msg::TH12EndingMsg2Txt(file_data, data);
		} else {
			return Touhou::Msg::TH12Msg2Txt(file_data, data);
		}
	} else {
...
bool GetList(std::vector<std::shared_ptr<LIST> > &list, std::shared_ptr<FILE> fp) {
	...
	if(!TryVerFileExtract(DAT_VAR_TH12, list, fp)) {
		return false;
	}

	if(!TryAnmFileExtract(DAT_VAR_TH12, list, fp)) {
		return false;
	}
	return true;
}
