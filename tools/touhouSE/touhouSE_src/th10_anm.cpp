...
private:
  ...
  public:
    endian::ulittle32_t count1;
    endian::ulittle32_t count2;
    endian::ulittle32_t unknown1; // 必ず0
    endian::ulittle32_t width;
    endian::ulittle32_t height;
    endian::ulittle32_t type;
    endian::ulittle32_t unknown2; // 必ず0
    endian::ulittle32_t name_offset;
    endian::ulittle32_t unknown3; // 必ず0
    endian::ulittle32_t unknown4; // 必ず0
    endian::ulittle32_t unknown5; // 必ず4
    endian::ulittle32_t unknown6;
    endian::ulittle32_t texture_offset;
    endian::ulittle32_t exist_texture;
    endian::ulittle32_t next_offset;
    endian::ulittle32_t unknown7; // 必ず0
  };
...
public:
  ...
  static bool Read(std::istream &in, const unsigned long long int file_size, std::vector<FileRecord> &result) {
    ...
    do {
      ...
      in.seekg(base_addr);
      if (file_size < base_addr + sizeof(*header)) {
        return false;
      }
      ...
      if (!in.good() || file_size < base_addr + header->name_offset || file_size < base_addr + header->next_offset
        || header->unknown1 != 0 || header->unknown2 != 0 || header->unknown3 != 0 || header->unknown4 != 0
        || header->unknown5 != 4 || header->unknown7 != 0 || header->exist_texture > 1
        || (header->exist_texture == 1 && header->texture_offset == 0)
        || (header->exist_texture == 0 && header->texture_offset != 0)) {
        return false;
      }
      std::string name;
      ...
      if (header->exist_texture == 1) {
        result.push_back(FileRecord(name, base_addr, header));
      }
      base_addr += header->next_offset;
    } while(header->next_offset > 0 && base_addr < file_size && base_addr >= header->next_offset);
    return true;
  }
};
...
public:
  ...
  bool Read(std::istream &in, const unsigned long long int file_size, const FileRecord &record) {
    if (!in.good() || file_size < record.addr + record.header->texture_offset + sizeof(Header)) {
      return false;
    }
    in.seekg(record.addr + record.header->texture_offset);
    std::shared_ptr<Header> header = std::make_shared<Header>();
    in.read(reinterpret_cast<char *>(header.get()), sizeof(*header));
    if (!in.good() || file_size < record.addr + record.header->texture_offset + sizeof(Header) + header->length
