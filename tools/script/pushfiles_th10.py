# -*- coding: utf-8 -*-
"""pushfiles 作业 - th10 完整处理 (th09 同构: msg 是转换, 无二拆)
"""
import pathlib, shutil, subprocess, sys, json
sys.stdout.reconfigure(encoding='utf-8')

ROOT = pathlib.Path(r'E:\GitWorkspace\thworks')
SRC_DIR = ROOT / 'tsa' / 'th10'
OUT_ROOT = ROOT / 'pushfiles' / 'th10'
...
REL_BASE = ROOT / 'release' / 'tsa' / 'th10'
...
def main():
    ...
    print('=== th10 拆包 ===')
    out_main = OUT_ROOT / 'th10.dat'
    extract('th10.dat', out_main, 10)
    ...
    for rel in sorted(CONV):
        if rel.startswith('th10/'):
            convert_file(out_main, rel, 'th10')
        elif rel == 'thbgm.dat':
            thbgm_split(out_bgm, rel)
    print('=== th10 二拆 ===')
    for rel in sorted(SECOND):
        if rel.startswith('th10/'):
            second_level(out_main, rel, 'th10')
    print('=== th10 完成 ===')
