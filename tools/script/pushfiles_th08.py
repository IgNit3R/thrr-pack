# -*- coding: utf-8 -*-
"""pushfiles 作业 - th08 完整处理 (与 th07 同构)
源档案: th08.dat + thbgm.dat
"""
import pathlib, shutil, subprocess, sys, json
sys.stdout.reconfigure(encoding='utf-8')

ROOT = pathlib.Path(r'E:\GitWorkspace\thworks')
SRC_DIR = ROOT / 'tsa' / 'eiya'
OUT_ROOT = ROOT / 'pushfiles' / 'th08'
...
REL_BASE = ROOT / 'release' / 'tsa' / 'eiya'
...
def main():
    ...
    print('=== th08 拆包 ===')
    out_main = OUT_ROOT / 'th08.dat'
    extract('th08.dat', out_main, 8)
    ...
    for rel in sorted(CONV):
        if rel.startswith('th08/'):
            convert_file(out_main, rel, 'th08')
        elif rel == 'thbgm.dat':
            thbgm_split(out_bgm, rel)
    print('=== th08 二拆 ===')
    for rel in sorted(SECOND):
        if rel.startswith('th08/'):
            second_level(out_main, rel, 'th08')
    print('=== th08 完成 ===')
