# -*- coding: utf-8 -*-
"""pushfiles 作业 - th09 完整处理
th09 特殊: msg→txt 是转换(非二拆); 二拆 0
"""
import pathlib, shutil, subprocess, sys, json
sys.stdout.reconfigure(encoding='utf-8')

ROOT = pathlib.Path(r'E:\GitWorkspace\thworks')
SRC_DIR = ROOT / 'tsa' / 'th09'
OUT_ROOT = ROOT / 'pushfiles' / 'th09'
...
REL_BASE = ROOT / 'release' / 'tsa' / 'th09'
...
def main():
    ...
    print('=== th09 拆包 ===')
    out_main = OUT_ROOT / 'th09.dat'
    extract('th09.dat', out_main, 9)
    ...
    for rel in sorted(CONV):
        if rel.startswith('th09/'):
            convert_file(out_main, rel, 'th09')
        elif rel == 'thbgm.dat':
            thbgm_split(out_bgm, rel)
    print('=== th09 二拆 ===')
    for rel in sorted(SECOND):
        if rel.startswith('th09/'):
            second_level(out_main, rel, 'th09')
    print('=== th09 完成 ===')
