# -*- coding: utf-8 -*-
"""pushfiles 作业 - th095 完整处理 (th09 同构: msg 是转换, 无二拆)
"""
import pathlib, shutil, subprocess, sys, json
sys.stdout.reconfigure(encoding='utf-8')

ROOT = pathlib.Path(r'E:\GitWorkspace\thworks')
SRC_DIR = ROOT / 'tsa' / 'th095'
OUT_ROOT = ROOT / 'pushfiles' / 'th095'
...
REL_BASE = ROOT / 'release' / 'tsa' / 'th095'
...
def main():
    ...
    print('=== th095 拆包 ===')
    out_main = OUT_ROOT / 'th095.dat'
    extract('th095.dat', out_main, 95)
    ...
    for rel in sorted(CONV):
        if rel.startswith('th095/'):
            convert_file(out_main, rel, 'th095')
        elif rel == 'thbgm.dat':
            thbgm_split(out_bgm, rel)
    print('=== th095 二拆 ===')
    for rel in sorted(SECOND):
        if rel.startswith('th095/'):
            second_level(out_main, rel, 'th095')
    print('=== th095 完成 ===')
