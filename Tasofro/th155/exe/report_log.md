# 東方憑依華 (TH15.5) — th155_log.exe (观战/日志版) — 主程序深度静态分析报告

- 目标文件: `E:/GitWorkspace/thworks/tf/th155/th155_log.exe`
...
| 属性 | 值 |
|---|---|
| 大小 | 5288152 bytes (5.0 MiB) |
| MD5 | `d03f8bc55b76ed0ebeae3b8c34a39734` |
| SHA256 | `415bf6360947a108b28c435ca50ae8a5ffc51922404824ef1012a97ed1b09d53` |
| 文件修改时间 | 2020-01-26T12:12:52.442428+00:00 |
| PE 时间戳 | 2018-04-23 09:52:43 UTC (raw 0x5ADDACEB) |
...
| .rsrc | 0x4E4000 | 9280 | 5082112 | 9728 | 2.9042 | READ|IDATA | 0.954 |
| .reloc | 0x4E7000 | 190728 | 5091840 | 190976 | 6.6443 | READ|IDATA | 0.999 |
...
- 数字签名: 有 (offset 5282816, size 5336)
- Overlay: offset 5282816, size 5336 bytes (存在! 文件尾部附加数据)
...
按编码计数(节内扫描): ASCII=14814, SJIS(CP932)=26850, UTF-16LE=866
...
- `KERNEL32.DLL` [.rdata]
- `D:\Works\th155\th155_log.pdb` [.rdata]
- `WINMM.dll` [.rdata]
- `IMM32.dll` [.rdata]
- `d3dx11_43.dll` [.rdata]
- `d3dx9_43.dll` [.rdata]
...
## 12. 补充发现(专项深挖: 与 th155.exe 的关系)

- **同源码双构建,非补丁关系**: PDB 路径不同(`D:\Works\th155\th155.pdb` vs `D:\Works\th155\th155_log.pdb`),PE 时间戳相隔约 4 分钟(log 版 2018-04-23 09:52:43 先于主程序 09:56:22),导入表/节结构/入口点完全一致(EP 同为 RVA 0x2E1B8C)。
- **字节级差异仅 31 个区段(≈195KB)**: 主要集中于尾部 `.reloc`(两构建地址不同属正常)与各自独立的 Authenticode 签名;真正代码/数据差异点:
  - RVA 0x2E1F02 / 0x2E1F2D: .text 内两处 6 字节补丁(行为差异点);
  - 0x44DBB4~0x47016F: .rdata/.data 字符串区差异(~1.2KB);
  - manifest 差异: **th155_log.exe 请求 requireAdministrator**,主程序为 asInvoker。
- **独立签名**: log 版同样附 WIN_CERTIFICATE(offset 5,282,816, 5,336B),签发者同为 Tomohiko Nakajima。
- **定位**: th155_log.exe 为需要管理员权限运行的辅助/观战工具(名称 log = 观战日志/录播用),核心引擎与主程序相同。
